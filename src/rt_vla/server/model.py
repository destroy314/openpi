...
def _extract_observation_state(state: dict) -> np.ndarray:
    raw_state = state.get("state")
    if raw_state is None:
        raw_state = _extract_state_sequence(state)[0]
...
class OpenPiRTCTritonAdapter(BaseModelAdapter):
    ...
    def _warmup_policy(self) -> None:
        ...
        print("Warmup: compiling pi0.5 Triton sampling graph")
    ...
    def infer_actions(self, state: dict) -> list:
        image_type = list(self._key_mapping.keys())
        self.process_images(state, image_type)
        obs_state = self.process_actions_for_model(_extract_observation_state(state))
