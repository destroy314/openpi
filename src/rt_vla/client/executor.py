...
class RawActionExecutor(BaseExecutor):
    ...
    def _note_executed_action(
        self,
        *,
        timestamp: float,
        action: list[float],
        next_action: list[float] | None,
        raw_action: list[float] | None,
        raw_next_action: list[float] | None,
        interval_ms: float,
    ) -> None:
        self._last_executed_action = list(action)
        self._heartbeat_action = list(action)
        self._heartbeat_next_action = list(next_action if next_action is not None else action)
        self._raw_heartbeat_action = list(raw_action) if raw_action is not None else None
        if raw_next_action is None:
            self._raw_heartbeat_next_action = (
                list(self._heartbeat_next_action) if self._heartbeat_next_action is not None else None
            )
        else:
            self._raw_heartbeat_next_action = list(raw_next_action)
        self._heartbeat_started_at = float(timestamp)
        self._heartbeat_interval_ms = float(interval_ms)
        self._heartbeat_action_history.append({"timestamp": float(timestamp), "action": list(action)})
        self._exec_step_count += 1

    def _build_training_step_record(
        self,
        *,
        plan_id: str,
        chunk_index: int,
        ...
        return {
            ...
            "timestamp": float(timestamp),
            "command_action": list(command_action),
            "executed_action": list(executed_action),
    ...
    def execute_training_chunk(
        ...
        *,
        ...
        if not action_chunk:
            return []
        raw_actions = [[float(v) for v in action] for action in action_chunk]
        step_sleep_s = float(control_dt_s)
        if step_sleep_s <= 0.0:
            step_sleep_s = self.get_control_dt_s()
        step_interval_ms = max(0.0, step_sleep_s) * 1000.0
        records: list[dict] = []
        for chunk_index, raw_action_list in enumerate(raw_actions):
            if before_step_callback is not None:
                override_action = before_step_callback(chunk_index, list(raw_action_list))
                if override_action is not None:
                    raw_action_list = [float(v) for v in override_action]
            command_action_list = self.normalize_infer_actions([raw_action_list])[0]
            executed_action = self._command_transform.apply(command_action_list)
            timestamp = time.time()
            self.apply_action(np.asarray(command_action_list, dtype=np.float32).reshape(-1))
            with self._action_queue_lock:
                self._note_executed_action(
                    timestamp=timestamp,
                    action=command_action_list,
                    next_action=command_action_list,
                    raw_action=raw_action_list,
                    raw_next_action=raw_action_list,
                    interval_ms=step_interval_ms,
                )
            records.append(
                self._build_training_step_record(
                    plan_id=plan_id,
                    chunk_index=chunk_index,
                    timestamp=timestamp,
                    raw_action=raw_action_list,
                    command_action=command_action_list,
                    executed_action=executed_action,
                )
            )
            if after_step_callback is not None:
                after_step_callback(chunk_index, records[-1])
            if chunk_index + 1 < len(raw_actions) and step_sleep_s > 0.0:
        ...
        with self._action_queue_lock:
            ...
            if action is not None:
                ...
                self._note_executed_action(
                    ...
                    action=list(action),
                    next_action=list(next_action),
                    raw_action=list(raw_action) if raw_action is not None else None,
                    raw_next_action=list(raw_next_action) if raw_next_action is not None else None,
...
class OnDeviceMpcExecutor(BaseExecutor):
    ...
    def _build_training_step_record(
        ...
        *,
        ...
        by_source = {
            item.get("source"): list(item.get("action", []))
            for item in telemetry
            if isinstance(item, dict) and item.get("action") is not None
        }
        pre_mpc_action = by_source.get("pre_mpc_action", list(command_action))
        post_mpc_action = by_source.get("post_mpc_action", list(pre_mpc_action))
        executed_action = self._command_transform.apply(post_mpc_action)
        enriched_telemetry = [dict(item) for item in telemetry]
        enriched_telemetry.append(
            {
                "timestamp": float(timestamp),
                "action": list(executed_action),
                "source": "executed_action",
        ...
        return {
            ...
            "chunk_index": int(chunk_index),
            "timestamp": float(timestamp),
            "command_action": list(pre_mpc_action),
            "executed_action": list(executed_action),
            "raw_action": list(raw_action),
            "pre_mpc_action": list(pre_mpc_action),
            "post_mpc_action": list(post_mpc_action),
            "source": "execute_training_chunk_mpc",
    ...
    def execute_training_chunk(
        ...
        *,
        ...
        raw_actions = [[float(v) for v in action] for action in action_chunk]
        if before_step_callback is not None:
            overridden_actions: list[list[float]] = []
            for chunk_index, raw_action in enumerate(raw_actions):
                override_action = before_step_callback(chunk_index, list(raw_action))
                overridden_actions.append(
                    [float(v) for v in (override_action if override_action is not None else raw_action)]
                )
            raw_actions = overridden_actions
        command_actions = self.normalize_infer_actions(raw_actions)
        context = {
            "exec_step_at_request": self._exec_step_count,
            "predicted_idx": -1,
            "predicted_steps": 0,
        ...
        self.update_actions(command_actions, context, raw_action_list=raw_actions)
        ...
        observed_state = (
            list(self._last_executed_action)
            if self._last_executed_action is not None
            else list(command_actions[0])
        )
        for chunk_index, raw_action in enumerate(raw_actions):
            observation_timestamp = time.time() - max(0.0, self._state_delay_s) - 1e-6
            self.record_observation(observed_state, observation_timestamp)
            telemetry = self.tick(observed_state)
            ...
            observed_state = by_source.get("post_mpc_action", list(command_actions[min(chunk_index, len(command_actions) - 1)]))
            records.append(
                self._build_training_step_record(
                    plan_id=plan_id,
                    chunk_index=chunk_index,
                    ...
                    command_action=command_actions[min(chunk_index, len(command_actions) - 1)],
                    telemetry=telemetry,
                )
            )
            if after_step_callback is not None:
                after_step_callback(chunk_index, records[-1])
            if chunk_index + 1 < len(command_actions) and step_sleep_s > 0.0:
                time.sleep(step_sleep_s)
        return records
